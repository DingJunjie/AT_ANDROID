...
fun ReelPageDemo(navController: NavHostController, viewModelProvider: ViewModelProvider) {
    ...
    var currentDto = remember { BlogBaseDto() }
        ...
        when (horPage) {
            0 -> {
                VerticalPager(state = pagerState) { page -> // Our page content
                    ...
                    CuLog.debug(CuTag.Blog,"博文id：${currentDto.id}")
                    Box(modifier = Modifier.fillMaxSize()
                        ...
                        Column(modifier = Modifier.padding(end = 30.cdp, bottom = 30.cdp)
                            ...
                            LikeButton(currentDto.id,
                                ...
                                CuLog.debug(CuTag.Blog, "LikeButton回调：${currentDto.agrees}")
                                ...
                                CuLog.debug(CuTag.Blog, "修改后：${currentDto.agrees}")
                            ...
                            Spacer(modifier = Modifier.height(70.cdp))
                            CollectButton(currentDto.hasCollect, tintColor = Color.White) {

                            }
                            Spacer(modifier = Modifier.height(70.cdp))
                        }
                        if (state.value.flag < 0) {
                            Text(text = state.value.flag.toString())
                        }
                    }
                }
            }
