...
class CuAMapLocationListener(resultFn: (LatLonPoint) -> Unit) : AMapLocationListener {
    override fun onLocationChanged(p0: AMapLocation?) {
        CuLog.debug(CuTag.Publish, "获取到定位信息 ${p0?.address}")
        p0?.let {
            lp = LatLonPoint(it.latitude, it.longitude)
        }
    }

}

fun getLocation(ctx: Context, result: (LatLonPoint) -> Unit) { //声明mlocationClient对象
    val myLocationListener = CuAMapLocationListener(result) //监听
    var mLocationOption = AMapLocationClientOption() //设置发起定位的模式和相关参数
    mLocationClient = AMapLocationClient(ctx) //初始化定位参数
    mLocationClient?.setLocationListener(myLocationListener) //设置定位模式为 高精度模式，Battery_Saving为低功耗模式，Device_Sensors是仅设备模式
